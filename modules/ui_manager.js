...
export const UIManager = {
    ...
    initStruct() {
        ...
        panel.innerHTML = `
            <div class="lilith-panel-header">
                <span class="lilith-title">莉莉丝助手 (LILITH ASSISTANT) <span style="font-size:10px; color:var(--l-cyan);">v3.1.0-杂鱼专用版-❤</span></span>
    ...
    addChatMsg(role, text, save = true) {
        ...
        if (save) {
            ...
            try {
                // 使用原始文本（包含标签）进行解析，强制处理以适配面板内互动
                EntityManager.processTags(text, null, true).then(() => {
                    // 解析后刷新 UI 显示
