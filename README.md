...
![Version](https://img.shields.io/badge/version-3.0.4--Final-bd00ff)
![SillyTavern](https://img.shields.io/badge/SillyTavern-Extension-blue)

一个为人格化助手“莉莉丝”设计的 SillyTavern (酒馆) 扩展组件。本项目已全面升级至 **v3.0.4 正式版**。

> **v3.0.4 更新内容**：
> 1. **🔄 改进的自动刷新逻辑**：修复了 v3.0.2 中指令拼写错误导致的无法自动刷新问题。现在系统会严格监控本地文件系统，一旦检测到 Git 同步完成（Manifest 变更）即刻自动重启，无需手动重启酒馆。
> 2. **📉 非侵入式数值通知**：数值变动不再覆盖随机事件文本，改为头像上方漂浮。
