...
> 2. **� 保护标记系统**：引入 `[莉莉丝] ... [/莉莉丝]` 双闭合标记，完美解决酒馆正文渲染导致的回答泄露与正文污染问题。
> 3. **📱 现代 UI 重构**：采用“胶囊型”输入框、FontAwesome 专业图标库、优化的头像呼吸灯特效及更符合直觉的设置布局。
> 4. **🔄 深度同步联动**：悬浮面板与酒馆侧边栏设置实现双向实时同步，确保操作体验高度一致。
